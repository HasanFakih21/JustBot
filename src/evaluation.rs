...
use crate::tools::parameters::{
    mat_scale_base, mat_scale_bishop, mat_scale_knight, mat_scale_pawn, mat_scale_queen, mat_scale_rook,
};
...
impl Board {
    ...
    pub fn detect_repetition(&self) -> bool {
        let half_moves = self.state.plies_from_null.min(self.state.half_move_clock as usize);
        self.game_history
            .iter()
            .rev()
            .take(half_moves + 1)
