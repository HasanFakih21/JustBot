...
fn new_histogram() -> AtomicHistogram {
    AtomicHistogram::new(HIST_GROUPING_POWER, HIST_MAX_VALUE_POWER).expect("valid histogram configuration")
}
...
impl TrackedValue {
    ...
    pub fn record(&self, v: i64) {
        ...
        self.total_abs.fetch_add(v.unsigned_abs(), Ordering::Relaxed);
        #[allow(clippy::cast_sign_loss)]
        self.total_sq.fetch_add((v.saturating_mul(v)) as u64, Ordering::Relaxed);
...
pub fn dump_and_plot() {
    ...
    "buckets": ["#,
        ...
        for (j, b) in buckets.iter().enumerate() {
            ...
            write!(json, r#"{{"start":{},"end":{},"count":{}}}"#, b.start, b.end, b.count).unwrap();
        }
        json.push_str("]\n  }");
    }
    json.push_str("\n]\n");
...
macro_rules! track {
    ($name:expr; $v:expr) => {{
        #[allow(clippy::cast_possible_wrap)]
        {
            #[linkme::distributed_slice($crate::tools::stats::TRACKED_VALUES)]
            static ENTRY: $crate::tools::stats::TrackedValue = $crate::tools::stats::TrackedValue::new(const { $name });
            let value = $v;
