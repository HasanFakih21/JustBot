    ...
    LazyLock::new(|| {
        ...
        for depth in 1..128 {
            for move_count in 1..64 {
                // Quiet Moves
                table[1][depth][move_count] =
                    ((0.5595 + (move_count as f32).ln() * (depth as f32).ln() / 2.4581) * 1024.0) as i32;
                // Noisy Moves
                table[0][depth][move_count] =
                    ((0.1079 + (move_count as f32).ln() * (depth as f32).ln() / 3.0121) * 1024.0) as i32;
            }
        }
