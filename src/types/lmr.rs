    ...
    pub fn init(&mut self) {
        ...
        for depth in 1..128 {
            for move_count in 1..64 {
                // Quiet Moves
                self.base[1][depth][move_count] =
                    ((lmr_quiet_base + (move_count as f32).ln() * (depth as f32).ln() / lmr_quiet_div) * 1024.0) as i32;
                // Noisy Moves
                self.base[0][depth][move_count] =
                    ((lmr_noisy_base + (move_count as f32).ln() * (depth as f32).ln() / lmr_noisy_div) * 1024.0) as i32;
            }
        }
    }
}
