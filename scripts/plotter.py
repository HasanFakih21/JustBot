...
def bucket_to_x_index(bucket: dict) -> int:
    ...
    m = (s + e) / 2
    x = round(math.copysign(math.exp2(m), bucket["start"]))
    return x
...
def plot_tracked_value(
    ax, data: dict, index: int, min_bucket_fraction: float = 0.0
) -> None:
...
def plot_tracked_boolean(ax, data: dict, index: int) -> None:
    ...
    kw = {
        "arrowprops": {"arrowstyle": "-", "color": palette["text"], "lw": 1.5},
        "bbox": bbox_props,
        "zorder": 0,
        "va": "center_baseline",
        "color": palette["bg-a"],
